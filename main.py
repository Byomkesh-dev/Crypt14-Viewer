...
def query_messages_from_table_message(con: Connection, key_remote_jid: str, contacts: Dict[str, Optional[str]]) -> List[Message]:
    cur = con.cursor()
    # Performance fix:
    # 1. Join directly to `chat` + `jid` instead of the `chat_view` VIEW to avoid double-join overhead.
    # 2. Replace ORDER BY max(receipt_server_timestamp, received_timestamp) — which forces a full
    #    table scan and in-memory sort — with ORDER BY m.sort_id, which is WhatsApp's internal
    #    monotonic row-order key and works with the index on message(chat_row_id).
    query = """
            SELECT
                m.timestamp,
                ...
                m.message_type
            FROM message AS m
            INNER JOIN chat ON m.chat_row_id = chat._id
...
def query_all_chats(db_path: str, contacts: Dict[str, Optional[str]]) -> List[Chat]:
    ...
    if not table_messages_exists:
        # Create a temporary session index on message(chat_row_id) so per-chat lookups
        # run in microseconds instead of scanning 1M+ rows each time.
        # This index is NOT written to disk — it only lives for this connection.
        print("[+] Creating session index on message(chat_row_id) for faster lookup...")
        cur.execute("CREATE INDEX IF NOT EXISTS _tmp_msg_chat_idx ON message(chat_row_id, sort_id)")
        print("[+] Index ready")
